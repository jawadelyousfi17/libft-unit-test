...
int main()
{
    ...
    // test 1 : base case
    tests[0].test_case = '1';
    tests[0].result = ft_isalnum(tests[0].test_case);
    tests[0].expected = isalnum(tests[0].test_case);

    int i = 0;
    while (i < TESTS_NBR)
    {
        tests[i].test_case = i - 1;
        tests[i].result = ft_isalnum(tests[i].test_case);
        tests[i].expected = isalnum(tests[i].test_case);
        i++;
    }
